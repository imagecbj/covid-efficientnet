...
parser.add_argument('--wandb-entity', action='store_true', default='nuist')
parser.add_argument('--wandb-project', action='store_true', default='covid')
...
def main():
    ...
    # set attacker
    if args.mixbn and 'adv_' in args.model:
        
        attacker = PGDAttacker(args.attack_iter, args.attack_epsilon, args.attack_step_size, prob_start_from_clean=0.0)
    ...
    if args.rank == 0:
        ...
        if args.log_wandb:
            if has_wandb:
                wandb.init(project=args.wandb_project, name=exp_name, config=args, entity=args.wandb_entity)
            else: 
...
def train_one_epoch(
    ...
    adv = args.mixbn and 'adv_' in args.model
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            if adv:
                ...
                output = output.transpose(1, 0).contiguous().view(-1, args.num_classes)
                target = target.transpose(1, 0).contiguous().view(-1)
                
                batch_size = output.size(0)
                loss_main = loss_fn(output[:batch_size // 2], target[:batch_size // 2])
                loss_aux  = loss_fn(output[batch_size // 2:], target[batch_size // 2:])
            else:
                output = model(input)
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                if adv:
                    reduced_main_loss = reduce_tensor(loss_main.data, args.world_size)
                    losses_main.update(reduced_main_loss.item(), batch_size // 2)
                    
                    reduced_aux_loss = reduce_tensor(loss_aux.data, args.world_size)
                    losses_aux.update(reduced_aux_loss.item(), batch_size // 2)
            ...
            if args.local_rank == 0:
                if not adv:
                    ...
                    _logger.info(
                        'Train: {} [{:>4d}/{} ({:>3.0f}%)]  '
                        'Main Loss: {loss.val:>9.6f} ({loss.avg:>6.4f})  '
                        'Aux Loss: {aux_loss.val:>9.6f} ({aux_loss.avg:>6.4f})  '
                        'Time: {batch_time.val:.3f}s, {rate:>7.2f}/s  '
                        '({batch_time.avg:.3f}s, {rate_avg:>7.2f}/s)  '
                        'LR: {lr:.3e}  '
                        'Data: {data_time.val:.3f} ({data_time.avg:.3f})'.format(
    ...
    if not adv:
        ...
        return OrderedDict([('main_loss', losses_main.avg),('aux_loss', losses_aux.avg)])
